...
async def marketplace_page(request: Request, q: str = "", db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("marketplace.html", {
        "request": request, "current_user": current_user, "listings": listings, "q": q,
    })
...
async def marketplace_create_post(
    ...
    if photo and getattr(photo, "filename", None) and photo.filename:
        ...
        if ext in {"jpg", "jpeg", "png", "webp", "gif"}:
            ...
            dest = f"{MEDIA_DIR}/{fname}"
            with open(dest, "wb") as f:
                _sh.copyfileobj(photo.file, f)
...
async def marketplace_detail(request: Request, listing_id: int, db: Session = Depends(get_db)):
    current_user = get_current_user_from_cookie(request, db)
    ...
    return templates.TemplateResponse("marketplace_detail.html", {
        "request": request, "current_user": current_user, "listing": listing,
        "thread": thread, "is_seller": current_user and current_user.id == listing.seller_id,
    })

@app.post("/marketplace/{listing_id}/message")
async def marketplace_send_message(request: Request, listing_id: int,
...
async def marketplace_delete(request: Request, listing_id: int, db: Session = Depends(get_db)):
    current_user = get_current_user_from_cookie(request, db)
    if not current_user:
        return RedirectResponse("/login", status_code=302)
    listing = db.query(Listing).filter(Listing.id == listing_id).first()
